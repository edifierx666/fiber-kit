...
type (
  ...
  Config struct {
    ...
    ParseTokenFunc func(auth string, c *fiber.Ctx) (interface{}, error)
  }
  ...
  // ErrorHandlerWithContext  is almost identical to JWTErrorHandler, but it's passed the current context.
...
// Errors
var (
  ErrJWTMissing = fiber.NewError(http.StatusBadRequest, "missing or malformed jwt")
  ErrJWTInvalid = fiber.NewError(http.StatusUnauthorized, "invalid or expired jwt")
)
...
func JWT(cfgs ...*Config) fiber.Handler {
  ...
  extractors := createExtractors(config)
